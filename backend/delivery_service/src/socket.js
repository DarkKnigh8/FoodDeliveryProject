    ...
    socket.on('updateDeliveryStatus', async ({ deliveryId, status }) => {
      try {
        ...
        await Delivery.findByIdAndUpdate(deliveryId, { 
          ...
          // If status is 'delivered', also update delivery completion time
      ...
      try {
        ...
        if (delivery) {
          ...
          socket.emit('deliveryInfo', {
            deliveryId,
            data: delivery,
            // For now, we're sending hardcoded locations since DB doesn't have them yet
            restaurantLocation: { lat: 6.9300, lng: 79.8570 },
            customerLocation: { lat: 6.9330, lng: 79.8650 }
          });
        }
      } catch (err) {
